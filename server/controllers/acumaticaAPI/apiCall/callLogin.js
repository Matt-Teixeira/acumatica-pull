...
const callLogin = async (acumaticEndpoint) => {
    ...
    try {
        const res = await axios.post(acumaticEndpoint, {
            name: process.env.ENV === 'dev' ? process.env.DEV_LOGIN_NAME : process.env.PROD_LOGIN_NAME,
            Password: process.env.ENV === 'dev' ? process.env.DEV_LOGIN_PW : process.env.PROD_LOGIN_PW,
            company: process.env.ENV === 'dev' ? process.env.DEV_LOGIN_COMPANY : process.env.PROD_LOGIN_COMPANY,
        });
